  ...
  return (
    <div className="relative max-w-md w-full py-1" ref={dropdownRef}>
        ...
        >
          ...
          <ul className="max-h-96 overflow-y-auto">
            {displayItems.map((game) => (
              ...
              >
                <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 border border-gray-600">
                  <img
                    src={game.img}
                    alt={game.name}
                ...
                <div className="flex-1">
                  <p className="font-medium text-base">{game.name}</p>
                  {query.trim() && (
      ...
      {showResults && query.trim() && filteredGames.length === 0 && (
        <div
          className={`absolute top-full left-0 right-0 mt-2 p-8 text-center rounded-xl shadow-2xl
            ${isDarkMode ? "bg-gray-900 text-gray-400" : "bg-white text-gray-500"} border
            ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}
        >
          <p className="text-lg">No games found for "<strong>{query}</strong>"</p>
        </div>
      )}
    </div>
  );
