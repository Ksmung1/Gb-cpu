...
import goldstarImg from "../../../assets/images/small-goldstar.png"; // reuse weekly as placeholder
import othersImg from "../../../assets/images/small-goldstar.png";   // reuse twilight as placeholder

const groupImageMap = {
  goldstar: goldstarImg,
  others: othersImg,
};
...
const SupersusAdmin = () => {
  ...
  const toggleOutOfStock = async (item) => {
    const docRef = doc(db, "supersusProductList", item.id);
  ...
  const handleDelete = async (id) => {
    ...
    const success = await deleteItem({
      collectionName: "supersusProductList",
      id,
      db,
    });
  ...
  const handleHide = async (id, currentHide) => {
    if (!id) return;
    const docRef = doc(db, "supersusProductList", id);
  ...
  const handleSave = async () => {
    ...
    const docRef = doc(db, "supersusProductList", selectedItem.id);
    const { id, group, ...otherFields } = selectedItem;
  ...
  const handleAddNew = async () => {
    ...
    if (!id || !label || !rupees || !group || !resellerRupees) {
  ...
  const filteredItems = items.filter((item) => {
    if (activeGroup === "others") return item.group !== "goldstar";
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Super Sus Products</h2>
        <button
              ...
              <div className="flex gap-3 items-center">
                <div className="w-10 h-10 flex items-center justify-center">
                  <img
                    ...
                    src={groupImageMap[item.group] || othersImg}
                    alt={item.group}
                  />
                </div>
