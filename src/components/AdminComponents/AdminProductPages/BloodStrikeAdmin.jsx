...
import d from "../../../assets/images/105-golds.webp";
import dd from "../../../assets/images/320-golds.webp";
import specials from "../../../assets/images/bs-elite.webp";
...
const BloodStrikeAdmin = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      collection(db, "bloodStrikeProductList"),
      (snapshot) => {
  ...
  const toggleOutOfStock = async (item) => {
    const docRef = doc(db, "bloodStrikeProductList", item.id);
  ...
  const handleDelete = async (id) => {
    ...
    const success = await deleteItem({
      collectionName: "bloodStrikeProductList",
      id,
      db,
    });
  ...
  const handleHide = async (id, currentHide) => {
    if (!id) return;
    const docRef = doc(db, "bloodStrikeProductList", id);
  ...
  const handleSave = async () => {
    ...
    const docRef = doc(db, "bloodStrikeProductList", selectedItem.id);
  ...
  const filteredItems = items.filter((item) => {
    if (activeGroup === "others") return item.group !== "bloodstrike";
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Blood Strike Products</h2>
        <button
