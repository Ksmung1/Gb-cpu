...
import d from "../../../assets/images/60-monochrome.webp";
import dd from "../../../assets/images/60-monochrome.webp";
...
const ZenlessAdmin = () => {
  ...
  const toggleOutOfStock = async (item) => {
    const docRef = doc(db, "zenlessProductList", item.id);
  ...
  const handleDelete = async (id) => {
    ...
    const success = await deleteItem({
      collectionName: "zenlessProductList",
      id,
      db,
    });
  ...
  const handleHide = async (id, currentHide) => {
    if (!id) return;
    const docRef = doc(db, "zenlessProductList", id);
  ...
  const handleSave = async () => {
    ...
    const docRef = doc(db, "zenlessProductList", selectedItem.id);
  ...
  const filteredItems = items.filter((item) => {
    if (activeGroup === "others") return item.group !== "monochrome";
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Zenless Zone Zero Products</h2>
        <button
