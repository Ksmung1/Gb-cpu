...
import d from "../../../assets/images/80-tokens.webp";
import dd from "../../../assets/images/240-tokens.webp";
...
const HonorKingsAdmin = () => {
  ...
  const toggleOutOfStock = async (item) => {
    const docRef = doc(db, "honorKingsProductList", item.id);
  ...
  const handleDelete = async (id) => {
    ...
    const success = await deleteItem({
      collectionName: "honorKingsProductList",
      id,
      db,
    });
  ...
  const handleHide = async (id, currentHide) => {
    if (!id) return;
    const docRef = doc(db, "honorKingsProductList", id);
  ...
  const handleSave = async () => {
    ...
    const docRef = doc(db, "honorKingsProductList", selectedItem.id);
  ...
  const filteredItems = items.filter((item) => {
    if (activeGroup === "others") return item.group !== "tokens";
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Honor of Kings Products</h2>
        <button
...
export default HonorKingsAdmin;