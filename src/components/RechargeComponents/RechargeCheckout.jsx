...
const RechargeCheckout = ({
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      // Show Bharat UPI Tutorial (first time only, resellers only)
      if (
      ...
      // Normal flow (skip tutorial for non-resellers or if tutorial already shown)
      try {
        proceedToConfirm();
      } catch (err) {
    ...
    openModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const orderPromise = (async () => {
            if (selectedItem?.api === "smile") {
              ...
              if (!selectedItem.price && selectedItem.price !== 0) {
                console.warn(
                  `[RechargeCheckout] ⚠️ Missing price for Smile product: ${selectedItem.id} (${selectedItem.label})`
                );
              }
          ...
          await Promise.race([orderPromise, operationTimeout]);
          ...
          let errorMessage = `Order failed. Contact support → ID: ${newOrderId}`;
